...
export class DatabaseStorage implements IStorage {
  ...
  async updateYearbookPrice(yearbookId: string, newPrice: string, changedBy: string): Promise<{ success: boolean; message: string; yearbook?: Yearbook }> {
    ...
    await this.createNotification({
      ...
      message: `"${yearbook.title}" price updated to $${newPrice}`,
  ...
  async getAllPublishedYearbooks(schoolId: string): Promise<{ year: number; isPublished: boolean; price: string; frontCoverUrl: string | null }[]> {
    const result = await db.select({
      year: yearbooks.year,
      isPublished: yearbooks.isPublished,
      price: yearbooks.price,
      yearbookId: yearbooks.id
    ...
    const yearbookIds = result.map(y => y.yearbookId);
    ...
    return result.map(yearbook => ({ 
      ...
      frontCoverUrl: frontCoverMap.get(yearbook.yearbookId) || null
    }));
  }
