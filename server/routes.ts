...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Check email verification status for viewers (skip in test mode)
      if (user.userType === "viewer" && !user.isEmailVerified && process.env.TEST_MODE !== 'true') {
      ...
      if (user.userType === "school" && user.schoolId) {
        ...
        // Check if school email is verified (skip in test mode)
        if (!school.isEmailVerified && process.env.TEST_MODE !== 'true') {
