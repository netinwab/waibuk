...
import { insertUserSchema, insertSchoolSchema, insertMemorySchema, insertPublicUploadLinkSchema, insertSchoolGalleryImageSchema, passwordResetTokens } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (user.userType === "school" && user.schoolId) {
        ...
        if (!school.isEmailVerified) {
          ...
          return res.status(403).json({ 
            message: "Please verify your school email before logging in. Check your inbox for the verification link.",
            emailNotVerified: true,
            email: school.email
          });
        }
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    try {
      ...
      if (user && user.email) {
        ...
        await sendEmail(
          user.email,
          "Your Waibuk Password Was Changed",
          confirmationHtml
        );
      }
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      const emailResult = await sendEmail(email.toLowerCase(), "Verify Your Waibuk Account", emailHtml);
  ...
  app.post("/api/auth/school-register", upload.single('accreditationDocument'), async (req, res) => {
    try {
      ...
      await sendEmail(email, "Verify Your Waibuk School Registration", emailHtml);
    ...
    try {
      ...
      if (school) {
        ...
        if (existingBadges.length >= maxBadgeSlots) {
          return res.status(400).json({ message: `You have reached the maximum number of alumni badges (${maxBadgeSlots}). Please upgrade your account to add more alumni statuses.` });
        }
      }
