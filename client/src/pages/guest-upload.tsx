...
export default function GuestUpload() {
  ...
  const validateCode = async (codeToValidate: string) => {
    ...
    try {
      ...
      // Build URL with reCAPTCHA token for unregistered users
      let url = `/api/public-upload-links/${codeToValidate}`;
      if (!isLoggedIn && recaptchaToken) {
  ...
  const handleCodeSubmit = (e: React.FormEvent) => {
    ...
    // For unregistered users, require reCAPTCHA verification
    if (!isLoggedIn && !recaptchaToken) {
      setLinkError("Please complete the reCAPTCHA verification");
      return;
    }
  ...
  const uploadMutation = useMutation({
    mutationFn: async (data: GuestUploadForm) => {
      ...
      // Add reCAPTCHA token for unregistered users
      const storedUser = localStorage.getItem('user');
      const isLoggedIn = !!storedUser;
      if (!isLoggedIn) {
