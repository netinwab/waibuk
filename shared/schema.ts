...
export const notifications = pgTable("notifications", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id).notNull(),
  type: text("type").notNull(), // 'alumni_approved', 'alumni_denied', 'yearbook_purchase', 'upload_approved', 'system_announcement'
  title: text("title").notNull(),
...
export const cartItems = pgTable("cart_items", {
  ...
  year: integer("year").notNull(),
  price: text("price").default(VIEWER_YEAR_PRICE.toString()),
  orientation: text("orientation"), // 'portrait', 'landscape' - configured before adding to cart
  uploadType: text("upload_type"), // 'image', 'pdf' - configured before adding to cart
